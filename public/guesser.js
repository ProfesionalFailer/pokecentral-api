    ...
    constructor(id) {
        fetch("/pokemon/" + id)
            ...
            .then((mondata) => {
                ...
                this.abilities = Object.values(
                    mondata["abilities"][this.flat_name]
                ).join(", ");
                this.cry = mondata["cry"];
                this.dimension = `${mondata["height"][this.flat_name]}, ${
                    mondata["weight"][this.flat_name]
                }`;
...
function show_points() {
    document.getElementById("points_text").innerHTML =
        "Punti Totali: " + localStorage.getItem("total_points");
}
...
function guess() {
    ...
    if (
        !valid_mons.includes(
            flat_name(document.getElementById("user_guess").value)
        )
    ) {
...
function game_end() {
    ...
    if (points_now > 0) {
        ...
        new_points =
            parseInt(localStorage.getItem("total_points")) +
            parseInt(points_now);
    } else {
        message = "Hai perso!";
        new_points = 0;

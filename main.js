...
const pokemonlist = require(path.join(__dirname, "data", "lists", "pokemon.json"));
...
app.get("/pokemon/:id", cors(), (req, res) => {
	...
	res.end(
		JSON.stringify(
			JSON.parse(fs.readFileSync(path.join(__dirname, "data", "pokemons", String(id).padStart(4, "0") + ".json")))
		)
	);
});

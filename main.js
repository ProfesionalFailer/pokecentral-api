...
const pokemonlist = require(path.join(__dirname, 'data', 'pokemons.json'));
...
function flat_name(name) {
	return name.toLowerCase().replace(' ', '-').replace('♂', '').replace('♀', '-f');
}
...
app.get('/pokemon/:id', cors(), (req, res) => {
	...
	res.end(JSON.stringify(JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'pokemons', String(id).padStart(4, '0') + '.json')))));
});
...
app.listen(3128);
console.log('Listening at http://localhost:3128');
